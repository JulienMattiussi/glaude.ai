...
export default function Sidebar({
  ...
  const content = (
    ...
    >
      ...
      {conversations.some((c) => c.favorite) && (
        <>
          <Divider />
          <div className={`px-4 py-1 overflow-hidden transition-all duration-300 ${collapsed ? "h-0 opacity-0 py-0" : "opacity-100"}`}>
          ...
          <div className="px-2 flex flex-col gap-0.5">
            {conversations.filter((c) => c.favorite).map((conv) => (
              <ConversationItem
                ...
                onClick={() => { onSelectConversation(conv.id); if (isMobile) setCollapsed(true); }}
  ...
  return (
    <>
      ...
      {isMobile && !collapsed && createPortal(
        <div
